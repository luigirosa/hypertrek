 ...
 * 
 * 20091122: sistemata la gestione dei caratteri nei form e rimosse le vecchie chiamate mysqli
...
if (count($_POST) > 0) {
	...
	if ('update' == $_POST['azione']) {
		if (isset($_POST['xxx'])) {
			$qry = "DELETE FROM eventitrek WHERE idtimeline=$_POST[idtimeline]";
		} else {
			if ('' == $_POST['annodisplay']) $annodisplay = $_POST['anno']; else $annodisplay = $_POST['annodisplay'];
			$qry = "UPDATE eventitrek
			        SET anno='$_POST[anno]',annodisplay='$annodisplay',tordine='$_POST[tordine]',idicona='$_POST[idicona]',
			            evento='$evento',commento='$commento',lastmod=$lastmod
			        WHERE idtimeline=$_POST[idtimeline]";
		}
	} else {
		if ('' == $_POST['annodisplay']) $annodisplay = $_POST['anno']; else $annodisplay = $_POST['annodisplay'];
		if ('' == $_POST['idicona']) $idicona = 0; else $idicona = $_POST['idicona'];
		$qry = "INSERT INTO eventitrek
		        SET anno='$_POST[anno]',annodisplay='$annodisplay',tordine='$_POST[tordine]',idicona='$idicona',
		            evento='$evento',commento='$commento',lastmod=$lastmod";
	}
	$db->query($qry);
	$anno = $_POST['anno'];
...
echo "<p align='center'><b>Eventi Trek: $anno</b></p>";
echo "\n<table border='0'>";
echo "\n<tr><th>anno<br>annodisplay</th><th>ordine<br>icona</th><th>evento<br>commento</th><th>delete<br>&nbsp;</th></tr>";
...
if ($nuovoanno) {
	...
	$r = $db->query("SELECT tordine,annodisplay FROM eventitrek WHERE anno='$anno' ORDER BY tordine DESC LIMIT 1")->fetch_array();
	$neworder = $r['tordine'] + 10;
	$annodisplay = $r['annodisplay'];
	$q = $db->query("SELECT * FROM eventitrek WHERE anno='$anno' ORDER BY tordine");
	while ($r = $q->fetch_array()) {
		echo "\n<tr>";
		echo "\n<form method='post' action='tab_eventitrek.php?anno=$anno' target='main' name='tte-$r[idtimeline]'>";
		...
		echo "\n<td><textarea rows='4' cols='90' name='evento'>$x</textarea><br>";
		$x = normalizzaform($r['commento']);
		echo "\n<textarea rows='2' cols='90' name='commento'>$x</textarea></td>";
		// stile+delete
...
// nuovo record
echo "\n<tr>";
echo "\n<form method='post' action='tab_eventitrek.php' target='main' name='tte-new'>";
...
echo "</select>\n</td>";
// evento+commento
echo "\n<td><textarea rows='4' cols='90' name='evento'></textarea><br>";
echo "\n<textarea rows='2' cols='90' name='commento'></textarea></td>";
// stile+delete
echo "\n<td>&nbsp;<br>";
echo "\n<input type='submit' value='Ok'></form></td>";
echo "\n</tr>";
